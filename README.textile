...
* `pathToMusic` folder is scanned for new files (you can set this to your main music folder but there is no reason to trust iTunes in terms of not interfering with your precious files and tags)
* album art is attached on the fly (iTunes is used)
* // TODO check whether foobar2000 is used otherwise just sync with files
* tags (see `tagMappings` variable in `foo-tunes.js`) are modified (foobar2000 tagger via masstagger is used)
...
Set up foobar2000 converter component to convert music to `pathToMusic` folder (or file operations component if you do not want to convert - but I recommend using Nero AAC Encoder as aac provides better quality at a lower
